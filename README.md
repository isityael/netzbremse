...
- Woodpecker runs in-cluster directly against `isityael/netzbremse` on GitHub.
- Pushes to `main` run validation and `semantic-release`.
- `semantic-release` creates GitHub tags and releases directly on
  `https://github.com/isityael/netzbremse`.
- Version tags (`v*`) trigger Woodpecker image publishing to:
  - `ghcr.io/isityael/netzbremse-dashboard`
  - `ghcr.io/isityael/netzbremse-measurement`
